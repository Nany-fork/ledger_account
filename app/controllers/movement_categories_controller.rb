class MovementCategoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @movement_category.save
        ...
        format.json { render :show, status: :created, location: @movement_category }
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @movement_category.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def update
    respond_to do |format|
      if @movement_category.update(movement_category_params)
        ...
        format.json { render :show, status: :ok, location: @movement_category }
      else
  ...
  private
    ...
    def movement_category_params
      params.require(:movement_category).permit(:name)
    end
end
